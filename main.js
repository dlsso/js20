var DungeonCrawl = (function() {
	...
	var SMALL_DUNGEON = [room1, room2, room4, room7, room10, room11, room12] 
	...
	var init = function() {				// Creates the init method for DungeonCrawl
		...
		$('.'+currentRoom.number+'').on('transitionend', function(e) { console.log("transition ended"); console.log(currentRoom.number); console.log($('.'+currentRoom.number+''));
			$(document).on('keydown', function(key){
		...
		$('.'+4+'').on('transitionend', function(){ alert("transition end detected on 4");console.log($(currentRoom.number))})
